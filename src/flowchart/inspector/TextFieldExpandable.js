...
function TextFieldExpandable(props) {
  ...
  return (
      <Box sx={{display: 'flex', flexDirection: 'column', alignItems:'center', justifyContent:'center', ...style, width: '100%'}}>
        ...
        <Typography variant="h7" component="div" sx={{ flexGrow: 1, py:1, color: textColor, m:0, backgroundColor: primaryColor, width:'100%'}}>
            {label}
        </Typography>
            ...
            {value.map((val, index) => {
                return <Box sx={{display: 'flex', flexDirection: 'row', alignItems:'center', justifyContent:'space-between'}}>
                    ...
                    <Typography variant="h7" component="div" sx={{ textAlign:'start', flexGrow: 1, py:1, px:2, color: textColor, m:0}}>
                        Resposta {index+1}
                    </Typography>
                    <TextField 
                            ...
                            value={val}
                            onChange={(event) => {
                                handleFieldChange(props.data.name, [...value.slice(0, index), event.target.value, ...value.slice(index+1)]);
                            }}
                            />
                    <Icon sx={{color: textColor, cursor: 'pointer'}} onClick={() => {
